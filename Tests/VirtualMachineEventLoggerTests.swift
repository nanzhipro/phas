...
final class VirtualMachineEventLoggerTests: XCTestCase {
    ...
    func testLoggerAppendsStructuredEntries() throws {
        ...
        let layout = VirtualMachineBundleLayout(rootURL: temporaryRootURL.appendingPathComponent("vm-log.vmbundle", isDirectory: true))
        let record = makeRecord()

        try logger.append(event: .startRequested, summary: "Starting install boot.", record: record, layout: layout)
        try logger.append(event: .guestStopped, summary: "Guest powered off.", record: record, layout: layout)
