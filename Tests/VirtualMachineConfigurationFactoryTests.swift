...
final class VirtualMachineConfigurationFactoryTests: XCTestCase {
    ...
    func testInstallConfigurationAttachesISOAndCreatesNVRAM() throws {
        ...
        let record = makeRecord(state: .draft, bootSource: .installationImage, installImagePath: isoURL.path)
    ...
    private func makeFactory() -> VirtualMachineConfigurationFactory {
        let rootResolver = VirtualMachineBundleRootResolver(fileManager: fileManager, baseDirectoryURL: temporaryRootURL)
        let bundleStore = VirtualMachineBundleStore(fileManager: fileManager, rootResolver: rootResolver)
