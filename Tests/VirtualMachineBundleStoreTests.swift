...
final class VirtualMachineBundleStoreTests: XCTestCase {
  ...
  func testBootstrapCreatesBundleLayoutAndRoundTripsConfiguration() throws {
    ...
    let machineIdentifier = try store.machineIdentifierStore.loadMachineIdentifier(
  ...
  func testSparseDiskUsesRequestedLogicalSize() throws {
    let store = makeStore()
    let record = makeRecord(
      resources: VirtualMachineResources(cpuCount: 4, memoryMiB: 8 * 1024, diskGiB: 32))

    let layout = try store.bootstrapBundle(for: record)
    let resourceValues = try layout.diskImageURL.resourceValues(forKeys: [
  ...
  private func makeRecord(
    ...
    resources: VirtualMachineResources = VirtualMachineResources(
      cpuCount: 4, memoryMiB: 8 * 1024, diskGiB: 64)
