        ...
        if hasLiveSession {
            return VirtualMachineRecoveryEvaluation(
                ...
                report: VirtualMachineRecoveryReport(
                    ...
                    summary: "The VM is backed by a live runtime session, so the current state can be trusted.",
        ...
        case .running, .installing:
            ...
            return VirtualMachineRecoveryEvaluation(
                ...
                report: VirtualMachineRecoveryReport(
                    ...
                    summary: "The app relaunched without a live Virtualization session. The persisted transient state is no longer trusted, so the VM has been moved to Error until you inspect logs and retry or recover it to Stopped.",
                    actions: VirtualMachineRecoveryActions(
                        canRetryStart: true,
        ...
        case .error:
            return VirtualMachineRecoveryEvaluation(
                ...
                report: VirtualMachineRecoveryReport(
                    ...
                    summary: "Inspect the logs, then retry start or move the VM back to Stopped before continuing.",
                    actions: VirtualMachineRecoveryActions(
                        canRetryStart: true,
                        canRecoverToStopped: true,
                        canReloadFromDisk: true
