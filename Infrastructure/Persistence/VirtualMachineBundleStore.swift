...
struct VirtualMachineBundleRootResolver {
  ...
  func bundleLayout(for id: VirtualMachineID) -> VirtualMachineBundleLayout {
    let bundleURL = baseDirectoryURL.appendingPathComponent(
      "\(id.rawValue).vmbundle", isDirectory: true)
...
struct VirtualMachineBundleStore {
  ...
  func listRecords() throws -> [VirtualMachineRecord] {
    ...
    return
      ...
      .map { url in
        let data = try Data(
          contentsOf: url.appendingPathComponent("config.json", isDirectory: false))
        return try decoder.decode(VirtualMachineRecord.self, from: data)
      }
  }
  ...
  func deleteBundle(at bundleURL: URL) throws {
    ...
    guard standardizedURL.deletingLastPathComponent() == expectedParent,
      standardizedURL.pathExtension == "vmbundle"
    else {
  ...
  private func ensureBundleStructure(for record: VirtualMachineRecord) throws
    -> VirtualMachineBundleLayout
  {
    let layout = layout(for: record.id)
    ...
    try createSparseDiskIfNeeded(
      at: layout.diskImageURL, logicalSizeInBytes: record.resources.diskSizeBytes)
    try save(record, to: layout.configurationURL)
