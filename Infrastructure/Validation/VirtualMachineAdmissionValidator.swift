...
struct VirtualMachineAdmissionValidator: VirtualMachineAdmissionValidating {
    ...
    func validate(
        ...
        if !existingRecords.isEmpty {
            issues.append(blocking("single-vm", "phas MVP only supports one VM at a time. Delete the existing bundle before creating another."))
        }
        ...
        if request.resources.cpuCount < VirtualMachineResources.minimumCPUCount {
            issues.append(blocking("cpu-min", "CPU must be at least \(VirtualMachineResources.minimumCPUCount) vCPU."))
        }
        ...
        if request.resources.cpuCount > snapshot.maximumSafeCPUCount {
            issues.append(blocking("cpu-safe-max", "This host should keep at least one CPU core for macOS. Reduce the VM CPU count."))
        }

        if request.resources.memoryMiB > snapshot.maximumSafeMemoryMiB {
            issues.append(blocking("memory-safe-max", "This memory selection exceeds the safe threshold for the current host."))
        }
        ...
        if snapshot.availableDiskBytes > 0 && snapshot.availableDiskBytes < requiredDiskBytes {
            issues.append(blocking("disk-space", "Available disk space is too low for bundle creation and OS installation."))
        }
    ...
    private func inspectInstallImage(path: String) -> (distributionSupport: DistributionSupportLevel, issues: [VirtualMachineAdmissionIssue]) {
        let trimmedPath = path.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedPath.isEmpty else {
            return (.unverified, [blocking("iso-missing", "Choose a Linux ARM64 installation ISO before creating the VM.")])
        }
        ...
        guard fileManager.fileExists(atPath: url.path, isDirectory: &isDirectory), !isDirectory.boolValue else {
            return (.unverified, [blocking("iso-not-found", "The selected ISO path does not exist or points to a directory.")])
        }

        guard fileManager.isReadableFile(atPath: url.path) else {
            return (.unverified, [blocking("iso-unreadable", "The selected ISO is not readable by the app.")])
        }

        let allowedExtensions = ["iso", "img"]
        if !allowedExtensions.contains(url.pathExtension.lowercased()) {
            issues.append(blocking("iso-extension", "The selected file does not look like an install ISO or IMG."))
        }
        ...
        if architecture == .x86_64 {
            issues.append(blocking("iso-arch", "The selected image appears to target x86_64/amd64 instead of ARM64."))
        } else if architecture == .unknown {
            issues.append(warning("iso-arch-unknown", "Could not confirm that the selected image is ARM64 from its filename. Proceed only if you know it matches Apple silicon."))
        }
        ...
        if fileName.contains("ubuntu") {
            ...
            issues.append(warning("distribution-secondary", "Fedora Workstation ARM64 is part of the supplementary validation matrix, not the primary release target."))
        } else {
            supportLevel = .unverified
            issues.append(warning("distribution-unverified", "This image is outside the primary and secondary validation matrix. You can continue, but compatibility is not guaranteed."))
        }
    ...
    private func deduplicated(_ issues: [VirtualMachineAdmissionIssue]) -> [VirtualMachineAdmissionIssue] {
        var seen = Set<String>()
