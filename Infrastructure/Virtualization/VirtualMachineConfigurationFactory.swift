...
enum VirtualMachineConfigurationFactoryError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingInstallImage(let id):
            return "VM \(id.rawValue) is configured to boot from installation media, but no ISO path is available."
        ...
        case .unsupportedNetworkMode(let mode):
            return "The network mode \(mode.rawValue) is not supported by the current configuration factory."
        }
    }
}
...
struct VirtualMachineConfigurationFactory {
    ...
    init(
        ...
        self.variableStoreManager = variableStoreManager ?? EFIVariableStoreManager(fileManager: bundleStore.fileManager)
        self.lifecycleController = lifecycleController
        self.configurationValidator = configurationValidator
    }

    func prepareConfiguration(for record: VirtualMachineRecord) throws -> PreparedVirtualMachineConfiguration {
        let launchPlan = try lifecycleController.launchPlan(for: record)
        let layout = try bundleStore.ensureRuntimeArtifacts(for: record)
        let machineIdentifier = try machineIdentifierStore.loadOrCreateMachineIdentifier(at: layout.machineIdentifierURL)
        ...
        configuration.storageDevices = try makeStorageDevices(for: record, layout: layout, attachInstallImage: launchPlan.attachInstallImage)
    ...
    private func makeInstallMediaDevice(for record: VirtualMachineRecord) throws -> VZUSBMassStorageDeviceConfiguration {
        guard let path = record.installImagePath?.trimmingCharacters(in: .whitespacesAndNewlines), !path.isEmpty else {
    ...
    private func makeNetworkDevice(for record: VirtualMachineRecord) throws -> VZVirtioNetworkDeviceConfiguration {
