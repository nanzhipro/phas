...
final class VirtualMachineSession: NSObject, ObservableObject, VZVirtualMachineDelegate {
    ...
    func start() async throws {
        ...
        appendLog(.startRequested, summary: "Start requested using \(updatedRecord.bootSource.rawValue).")

        do {
            try await machine.start()
            virtualMachineState = machine.state
            appendLog(.startSucceeded, summary: "Virtual machine entered \(machine.state.logDescription).")
        } catch {
            let failedRecord = lifecycleController.recordByHandlingRuntimeFailure(updatedRecord, at: now())
            do {
    ...
    nonisolated func guestDidStop(_ virtualMachine: VZVirtualMachine) {
        Task { @MainActor in
            let updatedRecord = self.lifecycleController.recordByHandlingGuestStop(self.record, at: self.now())
            ...
            self.appendLog(.guestStopped, summary: "Guest stopped. Next boot source: \(updatedRecord.bootSource.rawValue).")
        }
    }

    nonisolated func virtualMachine(_ virtualMachine: VZVirtualMachine, didStopWithError error: Error) {
        Task { @MainActor in
            let updatedRecord = self.lifecycleController.recordByHandlingRuntimeFailure(self.record, at: self.now())
            self.record = updatedRecord
            self.virtualMachineState = virtualMachine.state
            self.latestSummary = error.localizedDescription
            do {
    ...
    nonisolated func virtualMachine(_ virtualMachine: VZVirtualMachine, networkDevice: VZNetworkDevice, attachmentWasDisconnectedWithError error: Error) {
        Task { @MainActor in
            let updatedRecord = self.lifecycleController.recordByHandlingNetworkDisconnect(self.record, at: self.now())
            self.record = updatedRecord
            self.virtualMachineState = virtualMachine.state
            self.latestSummary = error.localizedDescription
            do {
...
struct VirtualMachineSessionFactory {
    ...
    init(
        ...
        self.bundleStore = bundleStore
        self.configurationFactory = configurationFactory ?? VirtualMachineConfigurationFactory(bundleStore: bundleStore, lifecycleController: lifecycleController)
        self.lifecycleController = lifecycleController
        self.logger = logger
        self.now = now
    }
    ...
    func makeSession(for record: VirtualMachineRecord) throws -> VirtualMachineSession {
        ...
        try logger.append(
            ...
            summary: "Configuration prepared for \(preparedConfiguration.launchPlan.attachInstallImage ? "installation media" : "disk boot").",
            record: record,
...
private extension VZVirtualMachine.State {
